...
class IquaSoftenerData:
    ...
    water_shutoff_valve_installed: int
        ...
        with requests.Session() as session:
            ...
            if response.status_code == 401:
                ...
                raise IquaSoftenerException(
                    f"Invalid status ({response.status_code}) for data request. Reason ({response.text})"
                )
            ...
            return IquaSoftenerData(
                ...
                water_shutoff_valve_installed=int(data["waterShutoffValveInstalled"]["value"]),
            )
